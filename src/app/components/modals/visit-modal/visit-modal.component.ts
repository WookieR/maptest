...
export class VisitModalComponent  implements OnInit, ViewWillEnter, ViewWillLeave {
  ...
  async finishVisit() {
      ...
      // // PAGAR O AGREGAR PAGOS A LA COLA OFFLINE, COMENTAR ESTO PARA PROBAR FLOW OFFLINE SIN PAGAR FISICAMENTE
      try{
        const resp: any = await this.visitService.finishVisit([], 
                                                              this.newSimplePayments, 
                                                              this.client.sales[this.saleIndex].visit.id, 
                                                              commentary, 
